	...
	try {
		...
		if (body.deleted_diff_ids && body.deleted_diff_ids.length > 0) {
			for (const diffId of body.deleted_diff_ids) {
				statements.push(
					DB.prepare('DELETE FROM diffs WHERE id = ? AND profile_id = ?').bind(
						diffId,
						profileId
					)
				);
		...
		`
			...
			.bind(profileId, profileId, STORAGE_LIMITS.MAX_DIFFS)
			.run();
		...
		await DB.prepare(`UPDATE profiles SET ${updateClauses.join(', ')} WHERE id = ?`)
			.bind(...updateValues)
			.run();
		...
		const cfZoneId = (platform?.env as any)?.CF_ZONE_ID;
		const cfApiToken = (platform?.env as any)?.CF_API_TOKEN;
