...
export const GET: RequestHandler = async ({ params, platform }) => {
	...
	try {
		const profile = await DB.prepare(
			'SELECT id, diffs_hash, stars_hash, tldrs_hash, keys_hash, content_updated_at FROM profiles WHERE id = ?'
		)
			.bind(id)
			.first<Pick<ProfileRow, 'id' | 'diffs_hash' | 'stars_hash' | 'tldrs_hash' | 'keys_hash' | 'content_updated_at'>>();
