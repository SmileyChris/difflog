...
import type { EncryptedDiffRow, ProfileRow } from '../../../types';
...
export const GET: RequestHandler = async ({ params, platform }) => {
	...
	try {
		...
		// Get profile name
		const profile = await DB.prepare('SELECT name FROM profiles WHERE id = ?')
			.bind(diff.profile_id)
			.first<Pick<ProfileRow, 'name'>>();
