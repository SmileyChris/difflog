    ...
    const results = await Promise.allSettled(
      urls.map(async (url) => {
        ...
        return stories.slice(0, 20).map(story => {
          const discussionUrl = story.comments_url || `https://lobste.rs/s/${story.short_id}`;
          return {
            title: story.title || '',
  ...
  const results = await Promise.allSettled(
    subreddits.map(async (sub) => {
      ...
      return (data?.data?.children || []).map((post: any) => {
        ...
        const isRedditMedia = articleUrl && (
...
async function fetchGitHub(languages: string[]): Promise<FeedItem[]> {
  ...
  const results = await Promise.allSettled(
    langs.map(async (lang) => {
      ...
      const query = lang ? `created:>${since} language:${lang.toLowerCase()}` : `created:>${since}`;
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  try {
    ...
    const filterRecent = (items: FeedItem[]) => items.filter(item => !item.date || item.date >= cutoff);
