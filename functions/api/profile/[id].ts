...
import type { Env, ProfileRow, ProfileResponse } from '../../types';
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (includeData) {
      ...
      ).bind(id).all<{ encrypted_data: string }>();
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const allowedFields = ['name', 'languages', 'frameworks', 'tools', 'topics', 'depth', 'custom_focus', 'resolved_sources'];
    const setClauses: string[] = ['updated_at = datetime(\'now\')', 'failed_attempts = 0', 'lockout_until = NULL'];
    const values: any[] = [];
